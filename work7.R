...
# Group 'residence_since' into ranges
data$residence_group <- cut(
  data$residence_since,
  breaks = c(0, 2, 5, 10, Inf), # Define the ranges: 0-2, 3-5, 6-10, 11+
  labels = c("0-2 years", "3-5 years", "6-10 years", "11+ years"),
  include.lowest = TRUE
)

# Summarize data
residence_summary <- data %>%
  group_by(class, residence_group) %>%
  summarise(count = n(), .groups = "drop")

# Plot the relationship
ggplot(residence_summary, aes(x = residence_group, y = count, fill = class)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(
    title = "Relationship between Residence Duration Groups and Class Assignment",
    x = "Residence Duration (Grouped)",
...
# Group existing credits into meaningful ranges
data <- data %>%
  mutate(existing_credits_group = case_when(
    existing_credits == 1 ~ "1",
    existing_credits == 2 ~ "2",
    existing_credits == 3 ~ "3",
    existing_credits > 3 ~ ">3"
  ))

# Summarize data
credits_summary <- data %>%
  group_by(class, existing_credits_group) %>%
  summarise(count = n(), .groups = "drop")

# Plot the impact
ggplot(credits_summary, aes(x = existing_credits_group, y = count, fill = class)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(
    title = "Impact of Existing Credits on Class Balance",
    x = "Number of Existing Credits (Grouped)",
    y = "Number of Customers"
  ) +
  theme_minimal()
...
data <- data %>%
  mutate(loan_duration_group = case_when(
    duration <= 12 ~ "Short (<=12 months)",
    duration <= 36 ~ "Medium (13-36 months)",
    duration > 36 ~ "Long (>36 months)"
  ))

# Summarize data
checking_summary <- data %>%
  group_by(checking_status, loan_duration_group, class) %>%
  summarise(count = n(), .groups = "drop")

# Plot the data
ggplot(checking_summary, aes(x = checking_status, y = count, fill = class)) +
  geom_bar(stat = "identity", position = "fill") +
  facet_wrap(~loan_duration_group) +
  labs(
    title = "Variation of Checking Status with Loan Duration and Class",
    x = "Checking Account Status",
...
# Summarize data by savings_status, class, and mean credit_amount
savings_analysis <- data %>%
  group_by(savings_status, class) %>%
...
# Visualize the data
ggplot(savings_analysis, aes(x = savings_status, y = mean_credit_amount, fill = class)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "Mean Credit Amount by Savings Status and Class",
       x = "Savings Status",
...
# Summarize data by other_payment_plans and credit_amount
payment_plan_analysis <- data %>%
  group_by(other_payment_plans) %>%
  summarise(mean_credit_amount = mean(credit_amount, na.rm = TRUE),
            median_credit_amount = median(credit_amount, na.rm = TRUE),
            count = n())

# Visualize the data
ggplot(payment_plan_analysis, aes(x = other_payment_plans, y = mean_credit_amount, fill = other_payment_plans)) +
  geom_bar(stat = "identity") +
  labs(title = "Mean Credit Amount by Other Payment Plans",
...
ggplot(data, aes(x = factor(class), y = num_dependents, color = factor(class))) +
  geom_boxplot() +
  labs(title = "Impact of Number of Dependents on Credit Class",
       x = "Credit Class",
...
# Assuming 'own_telephone' is a binary variable (1 = Yes, 0 = No) and 'class' is categorical
# Barplot for visualizing
ggplot(data, aes(x = factor(own_telephone), fill = factor(class))) +
  geom_bar(position = "dodge") +
  labs(title = "Impact of Telephone Ownership on Credit Class",
       x = "Own Telephone (0 = No, 1 = Yes)",
       y = "Count of Individuals") +
  theme_minimal()

# if ownership of a telephone is associated with credit class
chisq_result <- chisq.test(table(data$own_telephone, data$class))
chisq_result
